...
class Kriteria extends CI_Controller {
    ...
    public function index()
    {
        ...
        $this->load->view('manual/kriteria/index', $data);
    ...
    public function kriteria_masuk($jum)
    {
        ...
        if ($cek_jumlah == 0) {
            ...
            if ($insert == 1) {
                ...
                redirect('manual/proses/proses_kriteria');
            } else {
                echo "Gagal Insert";
                redirect('manual/kriteria');
            }
        } else {
            for ($i = 0; $i < $jum; $i++) {
                $data = [
    ...
    public function config()
    {
        ...
        $data = [
            ...
            'title' => $title,
            'nama_user' => $nama_user,
            'navigasi' => $this->navigasi('<a href="' . base_url('manual/kriteria') . '">Perbandingan Kriteria</a> / ' . $title),
        ];
        ...
        $this->load->view('templates/sidebar', $data);
		$this->load->view('templates/topbar', $data);
        $this->load->view('manual/kriteria/config_kriteria', $data);
        $this->load->view('templates/footer');
